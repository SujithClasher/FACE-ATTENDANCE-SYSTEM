...
class VideoStream:
    def __init__(self, stream):
        ...
        # Setting the frame size of the video stream
        ...
        for f in fnames:
            if f.endswith(".jpg") or f.endswith(".png"):
                ...
                if len(face_locations) == 0:
                    print(f"No face detected in {f}, skipping...")
                    continue
                encoding = fr.face_encodings(face, face_locations)[0]
...
def attendance(name_):
    ...
    with open(f'Attendance/Attendance_Of_{today}.csv', 'r') as f:
        data = f.readlines()
        names = []
        for line in data:
            entry = line.split(',')
    ...
    with open(f'Attendance/Attendance_Of_{today}.csv', 'a') as fs:
        if name_ not in names:
            current_time = time.strftime('%I:%M:%S')
            if name_ not in exclude_names:
                fs.write(f"\n{name_}, {current_time}, Present")
